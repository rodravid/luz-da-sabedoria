...
<head>
    ...
    <title>@yield('title', 'CMS Luz da Sabedoria' . (isset($currentModule) ? ' - ' . $currentModule->getTitle() : ''))</title>
            ...
            $('body').delegate('[data-form-link]', 'click', function(e) {
                ...
                function submitForm(inputValue)
                {
                    ...
                    if (typeof inputValue !== 'undefined') {
                        $form.append('<input type="hidden" name="inputValue" value="' + inputValue + '">');
                    }
                ...
                var inputText = $self.data('input-text');
                var inputRequired = Boolean($self.data('input-required'));
                var isPrompt = Boolean($self.data('prompt'));
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('[data-countdown]').each(function() {
                ...
                var $el = $(this);
                var expiration = $el.attr('data-time');

                if (expiration != '') {
                    if (moment(expiration).isAfter(moment())) {
                        $el.countdown(expiration, function (event) {
                            $(this).text(event.strftime('%M min %S seg'));
                        }).on('finish.countdown', function () {
                            $el.removeClass().addClass('badge').text('Esgotado');
                        });
                    ...
                    $el.removeClass().addClass('badge').text('Não se aplica');
                }
