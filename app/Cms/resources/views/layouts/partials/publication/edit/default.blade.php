<div class="box box-primary">
    ...
    <div class="box-footer">
        ...
        @endif
        @if($model->hasProperty('blacklist'))
            <a href="javascript:void(0);" class="btn btn-dark btn-block"
               data-form-link
               data-confirm-title="Confirmação de alteração"
               @if(! $model->isOnBlacklist())
               data-confirm-text="Atualmente este cliente <strong>não está</strong> na lista negra. Deseja realmente coloca-lo na lista?"
               @else
               data-confirm-text="Atualmente este cliente <strong>está</strong> na lista negra. Deseja realmente retira-lo da lista?"
               @endif
               data-method="POST"
               data-action="{{ route('cms.customers.edit#blacklist.toggleStatus', ['customer' => $model->id]) }}">
                <i class="glyphicon glyphicon-star"></i> @if(! $model->isOnBlacklist()) Colocar na lista negra @else Retirar da lista negra @endif
            </a>
        @endif
